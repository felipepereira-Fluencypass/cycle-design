// AUTO-GENERATED — não edite manualmente
// Fonte: src/icons/_source → npm run build:icons
import { BaseIcon } from '../BaseIcon'
import type { IconProps } from '../types'

/**
 * @example
 * // Decorativo (ícone puramente visual)
 * <DotLiveIcon size="sm" decorative />
 *
 * // Semântico (requer aria-label)
 * <DotLiveIcon size="sm" aria-label="descrição do ícone" />
 */
export function DotLiveIcon(props: IconProps): JSX.Element {
  return (
    <BaseIcon {...props}>
      <path fill="#181D27" stroke="#181D27" strokeWidth="1.5" d="M12 18a6 6 0 1 0 0-12 6 6 0 0 0 0 12Z"/>
    </BaseIcon>
  )
}
